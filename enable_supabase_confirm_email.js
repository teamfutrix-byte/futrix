...
async function main() {
  ...
  if (!page) {
    console.error("Supabase Auth providers page not found in active browser tabs!");
  ...
  const result = await page.evaluate(() => {
    ...
    if (switchBtn) {
      ...
      if (!isChecked) {
        switchBtn.click();
        return { success: true, toggled: true, msg: 'Toggled Radix switch ON' };
      } else {
        return { success: true, toggled: false, msg: 'Radix switch already ON' };
      }
    } else if (input) {
      const isChecked = input.checked;
      if (!isChecked) {
        input.click();
        return { success: true, toggled: true, msg: 'Clicked checkbox input ON' };
      } else {
        return { success: true, toggled: false, msg: 'Checkbox input already ON' };
      }
    }
  ...
  if (result.success && result.toggled) {
    console.log("Waiting 1.5s before saving...");
    await new Promise(resolve => setTimeout(resolve, 1500));

    console.log("Clicking 'Save changes' button...");
    const saveClicked = await page.evaluate(() => {
      const buttons = Array.from(document.querySelectorAll('button'));
      const saveBtn = buttons.find(b => b.innerText.includes('Save changes') || b.innerText.includes('Save'));
      if (saveBtn) {
    ...
    if (saveClicked.success) {
      console.log("Waiting 4s for save to complete...");
